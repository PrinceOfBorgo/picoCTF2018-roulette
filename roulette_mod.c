...
/* Returns a signed long but works locally with an unsigned int64 */
long get_long() {
    printf("> ");
    uint64_t l = 0;
    char c = 0;
...
/* This procedure is no more needed */
...
void play_roulette(long choice, long bet) {
    ...
    /* Show spin result without playing animation */
    printf("spin: %ld\n",spin); 
    //spin_roulette(spin);
    ...
    if (spin == choice) {
        ...
        /* The next line is commented because we want the same number of rand() calls as in "win" branch */
...
int main(int argc, char *argv[]) {
    /* Pass seed as first argument */
    if (argc >= 2){
        setvbuf(stdout, NULL, _IONBF, 0);

        /* Set cash and seed for rand() calls to the value of first argument */
        long seed;
        sscanf(argv[1], "%ld", &seed);
        srand(seed);
        cash = seed;

        /* Next line is commented because we don't want to call get_rand(): it would set seed to a random value */
        //cash = get_rand();
